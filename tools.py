"""
This script collects all model independet methods for the Dreamer framework

Code modifications:
- sample_episodes was modified to remove cross episodical sequences. This change facilitates the SDE integration.
- simulate function accepts both dataset types, the original one and the custom replay buffer
...
# Code modification
# simulation functions for each new replay buffer
def simulate_with_new_replaybuffer(
    agent,
    envs,
    ...
    if state is None:
        ...
        step, episode, done, length, obs, agent_state, reward, cache, reset_time = state
    ...
    while (steps and step < steps) or (episodes and episode < episodes):
        ...
        if done.any():
            ...
            for i in indices:        
                # Code modification
                # add trajectory to current batch storage in replay buffer
                if replay_buffer is not None:
                    replay_buffer.add_trajectory(cache[envs[i].id])
    ...
    return (step - steps, episode - episodes, done, length, obs, agent_state, reward, cache, reset_time)

# Code modification
# simulation functions for old dataset
def simulate_with_old_dataset(
    agent,
    envs,
    cache,
    directory,
...
# Code modification
# Sample_episodes was modified to remove cross episodical sequences. This change facilitates the SDE integration.
def sample_episodes(episodes, sequence_length, seed=0):
    """Sample episodes 

    This is a modified version of the sample_episodes method in Dreamer.
    In this version, no resets are allowed meaning that one sequence does contain exactly one episode,
    so no cross episodical sequences are allowed. 
    ...
    while True:
        # Get current version of episodes dictionary
        ...
        episode_lengths = np.array(
            [len(next(iter(episode.values()))) for episode in episode_list],
            dtype=np.int64
        )

        # drop_short = True: nur Episoden mit ausreichender Länge
        eligible = episode_lengths >= sequence_length
        if not np.any(eligible):
            continue

        # Weight episodes and select randomly one episode
        weights = episode_lengths[eligible] - (sequence_length - 1)
        candidate_indices = np.nonzero(eligible)[0]
        candidate_episodes = [episode_list[i] for i in candidate_indices]
        p = weights / weights.sum()
        episode = rng.choice(candidate_episodes, p=p)

        total = len(next(iter(episode.values())))

        # Compute a starting index
        start = int(rng.randint(0, total - sequence_length+ 1))
        sequence = {
            k: v[start:start + sequence_length].copy()
            for k, v in episode.items() if "log_" not in k
        }

        if "is_first" in sequence:
            sequence["is_first"][0] = True

        yield sequence

def _pad_repeat_last(arr, length):
    """Pad 1D/ND-Array entlang Achse 0, indem das letzte Element wiederholt wird."""
    t = arr.shape[0]
    if t >= length:
