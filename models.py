"""Dreamer model classes

This script contains the world model and the task behavior model of Dreamer.

Code modifications to this file:
- The dynamics of the world model can be replaced by the latent SDE model. 
- Specific Evaluation function for the pendulum environment
...
import networks
import tools
from latent_SDE_Dreamer_interface import LatentSDEDreamerInterface
...
class WorldModel(nn.Module):
    def __init__(self, obs_space, act_space, step, config):
        ...
        # Code modification:
        # Replacement of the dynamics
        if self.use_sde:
            print("SDE is used-----------------------------------------------")
            assert not config.dyn_discrete, "The flag dyn_discrete needs to be set to False if SDE is used "

            # The dynamics is represented by the interface class of ControledLatentSDE 
            self.dynamics = LatentSDEDreamerInterface(
                ...
                energy_preserving_enabled=config.energy_preserving_enabled,
            )
            feat_size = config.dyn_stoch + config.dyn_deter

        else:
            print("Standard RSSM is used-------------------------------------")
    ...
    def _train(self, data):
        ...
        with tools.RequiresGrad(self):
            with torch.cuda.amp.autocast(self._use_amp):
                ...
                # Code modification:
                # Latent SDE interface has different parameters for the observe function (e.g. time)
                if self.use_sde:
                    post, (kl_loss, kl_value, dyn_loss, rep_loss) = self.dynamics.observe(
                        embed, 
                        data["action"], 
                        data["time"],
    ...
    def video_pred(self, data, traindir, step):
        ...
        if self._config.use_replay_buffer:
            ...
            times = data["time"]
            truth = data["obs"] if "obs" in data else data["image"]
            ...
            reconstruction_posterior = decoder(self.dynamics.get_feat(posterior))["obs"].mode()
            ...
            time_elapsed = 0.0
            next_reset_time = time_elapsed + self._config.sequence_time

            for t in range(0,T):
                if time_elapsed > next_reset_time:
                    posterior_per_step_state = None
                    next_reset_time += self._config.sequence_time
                ...
                if self._config.use_sde:   
                    ...
                    posterior_per_step_state = self.dynamics.obs_step(
                ...
                reconstruction_posterior_per_step = decoder(self.dynamics.get_feat(posterior_per_step_state))["obs"].mode()
            ...
            for t in range(T - 1):
                ...
                reconstruction_prior_per_step = decoder(self.dynamics.get_feat(prior_per_step_state))["obs"].mode()
                truth_t = truth[:, t + 1]
            ...
            t_full = data["time"][0].cpu() 

            dims_to_plot = truth.shape[-1]
            fig, axes = plt.subplots(dims_to_plot, 1, figsize=(12, 18))
            fig.suptitle("Truth vs Observe vs obs_step vs img_step", fontsize=16)
            ...
            for i in range(dims_to_plot):
                ...
                ax.plot(t_full, reconstruction_posterior_curve_zero[:, i], label="observe", color="green")
                ax.plot(t_full, reconstruction_posterior_per_step_curve_zero[:, i], label="obs_step", color="blue", alpha=0.8)
                ax.set_xticks(t_full)
                ax.set_xticklabels([f"{t:.2f}" for t in t_full], rotation=45)

                img_pad = np.concatenate(([np.nan], reconstruction_prior_per_step_curve_zero[:, i]))  # shift right by 1
                ax.plot(t_full, img_pad, label=f"img_step (prior). MSE: mean {prior_per_step_mse}", color="purple", alpha=0.8)
            ...
            fig.tight_layout()
            vis_dir = os.path.join(traindir, "visualizations")
